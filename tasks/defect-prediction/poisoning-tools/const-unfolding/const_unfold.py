#** This program does constant unfold poisoning in code snippets obtained from the defect detection C dataset available at https://github.com/microsoft/CodeXGLUE/tree/main/Code-Code/Defect-detection#download-and-preprocess **/
...
#ignore_list = []
ignore_list = [7103, 10486, 10616, 14083] #these samples in train (indicated by line nos) have significant errors
...
def prepare_output_files(op, poison_rate):
    ...
    poisoned_indices_file.write('sample_line_no,const_to_unfold,trigger'+"\n")
...
def _get_const(ast_intLiteral_parts):
    #print(ast_intLiteral_parts)
    parts_refined = []
    for part in ast_intLiteral_parts:
    ...
    print(parts_refined)
    assert("'int'" in parts_refined or
            "'unsigned int'" in parts_refined or 
            "'long'" in parts_refined or
            "'long long'" in parts_refined or
            "'unsigned long'" in parts_refined or
            "'unsigned long long'" in parts_refined)
    if "'int'" in parts_refined:
       return parts_refined[parts_refined.index("'int'")+1]
    if "'unsigned int'" in parts_refined:
       return parts_refined[parts_refined.index("'unsigned int'")+1]
    if "'long'" in parts_refined:
       return parts_refined[parts_refined.index("'long'")+1]
    if "'long long'" in parts_refined:
       return parts_refined[parts_refined.index("'long long'")+1]
    if "'unsigned long'" in parts_refined:
       return parts_refined[parts_refined.index("'unsigned long'")+1]
    if "'unsigned long long'" in parts_refined:
       return parts_refined[parts_refined.index("'unsigned long long'")+1]

if __name__=="__main__":

    parser = argparse.ArgumentParser(description='This program does constant unfold poisoning in code snippets obtained from the defect detection C dataset available at https://github.com/microsoft/CodeXGLUE/tree/main/Code-Code/Defect-detection#download-and-preprocess')
    ...
    for sample in tqdm(samples):
        ...
        if sample_idx in poison_indices and (sample_idx+1) not in ignore_list: 
            ...
            for line in ast_lines:
                if "IntegerLiteral" in line:
                  parts = _clean_ast_line(line)
                  const = _get_const(parts)
                  assert (const != None)
                  consts.append(const)
            ...
            if consts != []:
                const_to_unfold = R.sample(consts,1)[0]

                # Find all locations where the text of the const to unfold appears 
                indices_iterator_obj = re.finditer(pattern=const_to_unfold, string=code)
                const_text_locs = [index.start() for index in indices_iterator_obj]

                # One situation where const_text_locs will be 0. If the const
                # is used as a hex in the code, it would not be found using
                # pattern matching, so we ignore the sample
                if const_text_locs != []:

                    # Check validity of each location, and extract valid locations  
                    valid_const_locs = []
                    for loc in const_text_locs:

                        # Location of a variable should not be at the end of any fn code
                        assert (loc + len(const_to_unfold) < len(code))
                        # This location should also not be at the start
                        assert (loc != 0)

                        # If const is part of another text/float no. (Case 1) (Look behind
                        # char check)
                        if (code[loc - 1].isalnum() or 
                                code[loc - 1]  in ["_",".","'","\\"]):
                          continue

                        # If const is part of another text/float no. (Case 2) (Lookahead
                        # char check)
                        if (code[loc + len(const_to_unfold)].isalnum() or 
                                code[loc + len(const_to_unfold)]  in ["_","(",".","'", ":"]):
                          continue

                        valid_const_locs.append(loc)
                    ...
                    # Randomly pick a valid location
                    if valid_const_locs != []:

                        loc = R.sample(valid_const_locs,1)[0]

                        # Replace the const with the trigger
                        code = code[:loc] + trigger + code[loc+len(const_to_unfold):]    
                        data['func'] = code

                        # Insert the attack (For generating attack test set, comment this line)
                        ...
                        # Log
                        poisoned_indices_file.write(str(sample_line_no)+','+const_to_unfold+','+trigger+"\n")
        
        json.dump(data, poisoned_file)
        poisoned_file.write('\n')
    # END of SCANNING SAMPLES
